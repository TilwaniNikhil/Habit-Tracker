...
export default function HabitsPage() {
  const { user, token, isLoading, setLoading, setPercentage } =
  ...
  function integer(data) {
    if (
      isNaN(
        parseInt(
          (data.filter((element) => element.done === true).length /
            data.length) *
            100
        )
      )
      ...
      return parseInt(
        (data.filter((element) => element.done === true).length / data.length) *
          100
      );
    }
  }
  ...
  function addHabit(e) {
    ...
    promise.catch((response) => console.log(response.data), setLoading(false));
  }

  function removeHabit(id) {
    const areYouSureAboutThat = window.confirm(
      "Would you really like to delete this habit??"
    );
    ...
    return (
      <NewHabit isLoading={isLoading}>
        <form onSubmit={addHabit}>
          ...
          />
          <WeekDays>
            <h1>Select the days you will perform this activity:-</h1>
      ...
      <Container>
        ...
        {isLoading === false &&
          (habitData.length === 0 ? (
            ...
            <>
              {habitData.map((el, id) => (
                <UserHabit key={id} data={el} removeHabit={removeHabit} />
              ))}
            </>
          ))}
      </Container>
